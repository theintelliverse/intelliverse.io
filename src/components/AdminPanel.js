...
export default function AdminPanel({ isOpen, data, testimonials: initialTestimonials, projects: initialProjects, submissions: initialSubmissions, admins: initialAdmins, currentUser, dbStatus }) {
  ...
  // --- Active Tab State ---
  const [activeTab, setActiveTab] = useState("dashboard"); // dashboard | cms | projects | testimonials | crm | settings
  ...
  const handleAddProject = () => {
    setProjects([...projects, { name: "", description: "", link: "", review: "", rating: 5 }]);
  };
  ...
  const handleDeleteAdmin = async (usernameToDelete) => {
    if (!window.confirm(`Are you sure you want to delete admin account '${usernameToDelete}'?`)) {
      return;
    }
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-950 font-sans text-white relative">
      ...
      {/* Sidebar Navigation */}
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-gray-900 border-r border-gray-800 flex flex-col z-40 transition-transform duration-300 ease-in-out md:translate-x-0 md:relative md:flex md:flex-col ${
        ...
        <nav className="flex-grow p-4 space-y-1 overflow-y-auto scrollbar-none">
          {[
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                activeTab === tab.id
      ...
      <main className="flex-grow p-4 md:p-10 relative overflow-y-auto max-h-[calc(100vh-65px)] md:max-h-screen z-10">
        ...
        {statusMessage.text && (
          <div className="fixed top-6 right-6 z-50 animate-slide-in shadow-2xl max-w-sm">
            ...
            >
              <i className={`fas ${
                statusMessage.type === "success" ? "fa-check-circle" : statusMessage.type === "error" ? "fa-exclamation-circle" : "fa-info-circle animate-pulse"
              } text-base`}></i>
        ...
        {activeTab === "settings" && (
          <div className="space-y-8 animate-fade-in">
            <div>
              <h2 className="text-xl md:text-2xl font-bold tracking-tight text-white">Manage Administrator Accounts</h2>
              <p className="text-xs md:text-sm text-gray-400 mt-1">Configure administrator login accounts and change passwords.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
              {/* Password Update Form */}
              <div className="bg-gray-900/40 border border-gray-800 p-5 md:p-8 rounded-2xl backdrop-blur-sm space-y-6">
                <h3 className="text-xs md:text-sm font-bold text-white pb-3 border-b border-gray-800">
                ...
                <form onSubmit={handleChangePassword} className="space-y-4">
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full sm:w-auto px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-bold transition cursor-pointer disabled:opacity-50"
                    >
                      Change Password
                    </button>
                  </div>
                ...
                <form onSubmit={handleCreateAdmin} className="space-y-4">
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full sm:w-auto px-5 py-2.5 bg-green-600 hover:bg-green-500 text-white rounded-lg text-xs font-bold transition cursor-pointer disabled:opacity-50"
                    >
            ...
            <div className="bg-gray-900/40 border border-gray-800 p-5 md:p-8 rounded-2xl backdrop-blur-sm space-y-4">
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 scrollbar-none text-xs">
                {adminsList.map((username, index) => {
                  ...
                  return (
                    <div key={index} className="flex justify-between items-center p-3 bg-gray-950/60 border border-gray-850 rounded-xl gap-2">
                      <div className="flex items-center gap-3">
                      ...
                      {canDelete ? (
                        <button
                          onClick={() => handleDeleteAdmin(username)}
                          disabled={loading}
                        ...
                        !isSelf && (
                          <span className="text-[10px] text-gray-650 italic text-right whitespace-nowrap">
                            Cannot delete last
                          </span>
                        )
                      )}
